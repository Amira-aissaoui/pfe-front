...
export class RulesComponent {
  ...
  pieChartColors: string[] = ['#00AE4E', '#F79027', '#A1CD3C', '#2BC3EA'];
  ...
  colorScheme: Color = {
    ...
    domain: ['#00AE4E', '#F36F2B', '#F1C40F', '#E74C3C']
  };
  ...
  constructor(private dataService:DataService,private location: Location,private dialog: MatDialog,private router: Router,
      private http: HttpClient, private activatedRoute: ActivatedRoute,private healthCheckService: HealthcheckService,private userInfoService:UserinfoService) {}
      isUserManager="";
  ...
  //    console.log("index"+this.microservicesnames);

//      console.log("index"+this.extractedIpAddr);
...
//----------
getActiveAlertRate(): number {
  if (this.nbAlerts === 0) return 0;
  const activeAlerts = this.alertArray.filter(alert => alert.state === 'active').length;
  return (activeAlerts / this.nbAlerts) * 100;
}


getInactiveRulesRate(): number {
  const inactiveRulesCount = this.rulesArray.filter(rule => rule.state !== 'active').length;
  const totalRulesCount = this.rulesArray.length;
  return (inactiveRulesCount / totalRulesCount) * 100;
}
getAlertSuccessRate(): number {
  if (this.nbAlerts === 0) return 0;
  const successfulAlerts = this.alertArray.filter(alert => alert.state === 'resolved').length;
  return (successfulAlerts / this.nbAlerts) * 100;
}
getAverageAlertsPerRule(): number {
  if (this.nbRules === 0 || this.nbAlerts === 0) return 0;
  return this.nbAlerts / (this.nbRules as number);
}



  getInstanceIndex(i:string){
   // console.log("indexLENAAAAAAAAA"+i);
...
//------------------
getStateDistribution(): { name: string, value: number }[] {
  const stateCounts = this.rulesArray.reduce((counts, rule) => {
    counts[rule.state] = (counts[rule.state] || 0) + 1;
    return counts;
...
//---------------
  ...
  generatePDF(): void {
    const doc = new jsPDF();
    const rules = this.rulesArray;
    doc.text('Rule List', 10, 10);
    doc.setTextColor(255, 255, 255); // White
    doc.setFontSize(12);
    // Set header background color to #F79027
    ...
    img.onload = () => {
      ...
      autoTable(doc, {
        head: [[ 'Instance', 'Name', 'Query', 'Duration', 'State']],
        body: rules.map(rule => [
        ...
        startY: 20,
        headStyles: {
          fillColor: [0, 174, 78], // #F79027
          textColor: [255, 255, 255] // White
        }
      });
      doc.save('rule_list.pdf');
    };
...
const tableHeight = 200;
   ...
   const centerX = pageWidth / 20;
   doc.setFont('Newsreader', 'normal')
   // Add the line at the bottom center
   doc.text('@ ACTIA 2024 Any reproduction or distribution of this document, even partial, is strictly prohibited without written permission', centerX, pageHeight - 1);
